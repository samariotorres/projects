...
X_set, y_set = X_train, y_train #create some local values so we can create a shortcut so that we dont have to go around and replace every
#single X_test and y_test. ;) 

#Line X1,X2 we take the minimum ages and estimatedsalaries - 1 and + 1 because we dont want the data points to be squeezed in
#graph and make it look ugly...we want some border room pretty much
X1, X2 = np.meshgrid(np.arange(start = X_set[:, 0].min() - 1, stop = X_set[:, 0].max() + 1, step = 0.01), #We took all of the pixel points 

                     np.arange(start = X_set[:,1].min() - 1, stop = X_set[:, 1].max() + 1, step = 0.01)) #available, and then we applied our classifier
...
plt.contourf(X1, X2, classifier.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),  #this line here is where we actually apply the
             #classifier to all of the "pixel observation points" and by doing that, it colorizes everything..contour function to actually make the
             #contour between the two prediction regions..
            
             alpha = 0.75, cmap = ListedColormap(('red','green'))) ------>if the pixel point belongs to class zero its get red, else it gets green

plt.xlim(X1.min(), X1.max()) #here we are plotting the limits of                               

plt.ylim(X2.min(), X2.max() #x, the age, and y, the estimatedsalary                              

for i, j in enumerate(np.unique(y_set)): #then with this for loop, we are plotting all of the data points that are the real values. (unique, separated)
